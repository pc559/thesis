...
though those methods are expected to generalise to this case.
...
meaning the dependence on each $k_i$ separately, for fixed $k_1+k_2+k_3$.
    ...
    inflation scenario by ruling out $\fnl=1$. We present here some of the ways
...
\textcolor{red}{are the $\pi$ factors correct?}
\begin{align}
S^{local}(k_1,k_2,k_3)
...
scaling similar to the spectral index $n_s$~\cite{Planck_NG_2015}.
...
\section{Step 3: the $\cmb$ bispectrum}
    ...
    \textcolor{red}{Check. Why $3$? Why does less then $200$ count as large?}
    ...
    come from a model of inflation. Note also that the acoustic oscillations and
...
Work on forecasting future constraints on features was performed
    ...
    \textcolor{red}{B vs b. Tidy all this up!}
    \begin{align}
        \left<\triplea\right>
    \end{align}
    ...
    \end{align}
    where the Wigner $3j$ symbol is demanded by isotropy.
    ...
    \textcolor{red}{Change estimator symbol!}
    \begin{align}
        \epsilon = \frac{1}{N}\sum_{l_i,m_i}\frac{\left<\triplea\right>\tripleaobs}{C_{l_1}C_{l_2}C_{l_3}}.
    \end{align}
    ...
    \begin{align}
        \epsilon = \frac{1}{\bar{N}^2}\sum_{l_im_i}
    ...
    \end{align}
    \textcolor{red}{Why? Is that what was calculated in the previous section?}


    \subsection{Complexity of bispectrum estimation}
    The bispectrum, like the power spectrum, is a quantity that describes
the statistical distribution of which our universe is a realisation.
We can use this one accessible sky to estimate the amplitude of
...
\textcolor{red}{Shorten, move to future work?}
