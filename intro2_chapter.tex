...
\end{table}
    ...
    The temperature anisotropies of the $\cmb$ can be decomposed in spherical
    ...
    \end{align}
    We can project the large scale primordial inhomogeneities onto the $\cmb$ sky,
    using the Rayleigh plane wave expansion and the addition theorem
    for spherical harmonics. Writing $\vecx=r\hat{\vecn}$,
    \begin{align}
        e^{i\veck\cdot\vecx} &= \sum_i i^l(2l+1)j_l(kr)P_l(\hat{\veck}\cdot\hat{\vecn})\\
                    &= 4\pi\sum_{lm}i^l j_l(kr)Y^*_{lm}(\hat{\veck})Y_{lm}(\hat{\vecn})\label{addition_expan}
    \end{align}
    ...
    The comoving distance to last scattering is $\chi_*$.
...
\begin{figure}[h!]
\centering
    \subfloat{\includegraphics[width=\columnwidth]{plots/tetrapyd_3d_colour_split.png}}
\caption{
...
meaning the dependence on each $k_i$ separately, for fixed $k_1+k_2+k_3$.
    ...
    In our template-free pipeline, we take $\fnl$ as a fitting coefficient of our expansion
...
\section{Step 3: the $\cmb$ bispectrum}
    ...
    then we can use the same large-scale Sachs-Wolfe approximation
    ...
    % See (3.47) of AC.
\textcolor{red}{Is the factor of $3$ correct?}
}.
