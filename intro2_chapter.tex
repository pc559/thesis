...
achucarro_quad_viability, achucarro_heavy_in_cmb, achucarro_gsr_cs_14, achucarro_cs_reduction_13,
...
\section{Perturbations about the background}
    ...
    \subsection{Quantizing the perturbations}
...
\begin{align}\label{inin_example}
    \left<\zeta_{k_1}(t)\zeta_{k_2}(t)\zeta_{k_3}(t)\right>&=
    \Re\left(\left<-2i \zeta^I_{k_1}(t)\zeta^I_{k_2}(t)\zeta^I_{k_3}(t)
    \int_{-\infty(1+i\varepsilon)}^{t}dt'\Hint^I(t')
    \right>\right).
\end{align}

To do this calculation we will use Wick's theorem
\begin{align}\label{wick}
    \left<0\left|\zeta_{k_1}\ldots\zeta_{k_n}\right|0\right>
    &= :\zeta_{k_1}\ldots\zeta_{k_n}:~+~\text{all contractions}.
\end{align}
where $:~:$ denotes normal ordering, which vanishes inside $<~>$.
We will also make use of the fact that, from~\eqref{operator_expansion},
\begin{align}
    \left<\zeta_{k_1}(t_1)\zeta_{k_2}(t_2)\right>
    &= \left<0\left|
            \left(u_{k_1}(\tau)\hat{a}_\mathbf{k_1} +
            u^*_{k_1}(\tau)\hat{a}^{\dagger}_{-\mathbf{k_1}}\right)
            \left(u_{k_2}(\tau)\hat{a}_\mathbf{k_2} +
            u^*_{k_2}(\tau)\hat{a}^{\dagger}_{-\mathbf{k_2}}\right)
        \right|0\right>\\
    &= \left<0\left|
            u_{k_1}(\tau)\hat{a}_\mathbf{k_1}
            u^*_{k_2}(\tau)\hat{a}^{\dagger}_{-\mathbf{k_2}}
        \right|0\right>\\
    &= u_{k_1}(\tau)u^*_{k_2}(\tau)\left<0\left|
            \left[\hat{a}_\mathbf{k_1},
            \hat{a}^{\dagger}_{-\mathbf{k_2}}\right]
        \right|0\right>\\
    &= u_{k_1}(\tau)u^*_{k_2}(\tau)(2\pi)^3\delta^{(3)}(\mathbf{k_1}+\mathbf{k_2})
\end{align}
...
\begin{align}\label{inin_example_start}
    \left<\zeta_{k_1}(t)\right.&\left.\zeta_{k_2}(t)\zeta_{k_3}(t)\right>\\
    &=\Re\left(\left<-2i \zeta_{k_1}(t)\zeta_{k_2}(t)\zeta_{k_3}(t)
...
\begin{align}\label{inin_example_2}
    \bigg<-2i \zeta_{k_1}(t)&\zeta_{k_2}(t)\zeta_{k_3}(t)
    \int_{-\infty(1+i\varepsilon)}^{t}dt'
    \int d^3x~\left(-2\lambda a^3\dzeta^3(\vecx,t')\right)
    \bigg>\\
    =&\left<-2i \zeta_{k_1}(t)\zeta_{k_2}(t)\zeta_{k_3}(t)
    \int_{-\infty(1+i\varepsilon)}^{t}dt'
    \int d^3x~\left(-2\lambda a^3\dzeta^3(\vecx,t')\right)
    \right>\\
    =&\bigg<-2i \zeta_{k_1}(t)\zeta_{k_2}(t)\zeta_{k_3}(t)
