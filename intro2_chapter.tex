...
\end{align}
At leading order in slow-roll, using definition~\eqref{spectrum_definitions},
the power spectrum is~\cite{mukhanov_1999,chen_ng_0605}
\begin{align}
P_{\zeta}(k) = \frac{H^2}{4\varepsilon c_sk^3},
\end{align}
where the right hand side is evaluated at $c_{s}k=aH$.
We also define a dimensionless version
\begin{align}
    \primpowerspec(k) = \frac{k^3}{2\pi^2}P_\zeta(k)
\end{align}
which is typically parameterised as
\begin{align}
    \primpowerspec(k) = A_s \left(\frac{k}{k_*}\right)^{n_s-1}.
\end{align}
The spectral index can be found to be
\begin{align}
...
\end{align}
    ...
    \begin{align}
        ...
        \rho,_X(X, \phi) &= \frac{1}{\left(1-f(\phi)X\right)^{\frac{3}{2}}}\\
