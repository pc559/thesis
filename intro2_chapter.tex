...
For a careful discussion, see Appendix A2 of~\cite{weinberg_in_in}---here,
we will simply sketch the derivation.
We can write this with reference to some initial time $t_i$, where the pictures coincide
\begin{align}
    \left|\psi,t\right>_S&=e^{-i\int_{t_i}^{t}\hat{H}dt}\left|\psi,t_i\right>_S\\
    \op_H(t) &= e^{i\int_{t_i}^{t}\hat{H}dt}\op_Se^{-i\int_{t_i}^{t}\hat{H}dt}
\end{align}
where $\op_S(t)=\op_S(t_i)$.
...
\begin{align}
    \left|\psi,t\right>_I&=e^{i\int_{t_i}^{t}\hat{H}_0dt}e^{-i\int_{t_i}^{t}\hat{H}dt}\left|\psi,t_i\right>_S\\
    \op_I(t) &= e^{i\int_{t_i}^{t}\hat{H_0}dt}\op_Se^{-i\int_{t_i}^{t}\hat{H_0}dt}.
\end{align}
...
free Hamiltonian, so their time dependence can be found. To take advantage of this,
...
using $\tilde{H}=H_0+H_I$
\begin{align}
    \delphi(t) &= U^{-1}(t,t_0)\delphi(t_0)U(t,t_0)\\
    \delpi(t) &= U^{-1}(t,t_0)\delpi(t_0)U(t,t_0)
\end{align}
which along with the Heisenberg equation of motion
(the explicit time dependence in $\tilde{H}$ comes from coefficients
...
\end{align}
where we have temporarily omitted the arguments of $U(t,t_0)$.
We then have
\begin{align}
...
which the Hamiltonian is built.
...
\end{align}
where we recall that $H_I$ is built from the interaction pictures fields, containing cubic
