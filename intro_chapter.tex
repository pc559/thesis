...
of a given theory) but will be designed with the fill pipeline in mind.
The models we will consider are models of inflation, and the object to be calculated will be the primordial bispectrum.
The methods for each step of this pipeline, for the case of inflationary bispectra, are described
...
as a whole, and
a practical implementation of that philosophy is the Modal methods. This thesis is
...
bispectrum, which is optimised for rough order of magnitude estimates of correlations, but not
for confrontation with the data.
We give further detail in Section~\ref{sec:config_codes}.
...
We explain how to swap the physical integral for a computationally cheaper one which evaluates to approximately the same result, we carefully show that we do not encounter the convergence problems of previous methods, and provide an intuition that explains the robustness of the method, in Section~\ref{sec:starting}.
