...
We will begin with an introduction to this thesis, before broadening the discussion to a summary of the general cosmological context in the remainder of this chapter. We will summarise the goals and state of the art of the field, and the main obstacles to these goals which are addressed by methods developed in this thesis, as well as our main results.

One important goal of cosmology is to probe the fundamental physics of the early universe. This is achieved by confronting models of the early universe with data gathered from telescopes. A crude sketch of the pipeline for this confrontation is as follows. From the theory-side, a model is proposed, and the observational consequences of the model are calculated. From the data-side, telescopes gather data, and statistics which describe this data are calculated. The predictions of the theory are compared with the data, and if the predictions are inconsistent with the data, the theory is ruled out.
...
A core motivation of the methods of separability and Modal methods (discussed in Section~\ref{sec:bis_est})
...
for bringing the full bispectrum information through the pipeline we sketched above, to be confronted by the data.
...
Another important obstacle is numerical efficiency, ensuring this calculation can be done sufficiently computationally cheaply that parameter scans become feasible. While the main factor in determining the efficiency of the methods developed in this thesis is the choice of basis set, the time integrals which must be performed to calculate the coefficients can be computationally expensive. These time integrals have an early-time contribution which is small and expensive, and a late-time contribution which is large and relatively cheaper.
...
We explain how to swap the physical integral for a computationally cheaper one which evaluates to approximately the same result, we carefully show that we do not encounter the convergence problems or previous methods, and provide an intuition that explains the robustness of the method, in Section~\ref{sec:starting}.
