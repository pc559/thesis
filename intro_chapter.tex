...
extract the fundamental physics waiting to be found in the universe's youth.
    ...
    The form of the FLRW metric (we will always set the speed of light to unity) is
    ...
    \begin{align}\eqref{continuity_equation}
        \dot{\rho_X} + 3\frac{\dot{a}}{a}\left(\rho_X+P_X\right) &= 0
    \end{align}
    ...
    \end{align}
    where $H$ is now constant, and a factor $\sqrt{\Omega_{\Lambda}}$ smaller than today.
...
\section{Outline of thesis}
    ...
    not point samples or a limit. The motivation for these goals was
    the desire for access to more accurate, and in some cases new, bispectrum shapes.
    ...
    for calculating the separable expansion we describe to high orders to sufficiently
    ...
    convergence of the expansion, and how it is affected by the non-physical $k$-configurations.
...
In chapter~\ref{chapter:methods} we present the precise methods involved in this pipeline.
...
we first, in chapter~\ref{chapter:decomp}, present this distinct and separate but nonetheless vital, discussion.
