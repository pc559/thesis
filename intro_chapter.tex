...
extract the fundamental physics waiting to be found in the universe's youth.
    ...
    making~\eqref{flrw_metric_tau} a function of time multiplied by the Minkowski metric.
    ...
    \end{align}
    This is a combination of the motion of an object with respect to the comoving coordinates
    $a(t)\frac{dr}{dt}$, and a term due to the expansion of the universe, which we see is linear
    ...
    We also have the continuity equation, which can be derived for each component
    ...
    We define the critical density $\rho_{crit,0}=\frac{3H^2}{8\pi G}$ as the (present) density for which the universe is flat.
    Using this,
    ...
    The evolution of the components is illustrated in Figure~\ref{fig:lcdm_components},
...
\end{figure}
    ...
    This is not evidence for thermal equilibrium,
...
\end{figure}
    In this $\Lambda$ dominated epoch
    \begin{align}
    ...
    We also find the asymptotic value of $\tau$ to be $19\Gpc$, indicating that we will never see the
...
\section{Initial conditions for $\lcdm$}
    ...
    We will not go into the details of this ``flatness problem'' except to state that
    a natural explanation of this would be an attractive quality in a
    theory of the very early universe.


    During the matter dominated era, $p=0$ and~\eqref{continuity_equation}
    gives us $a(\tau)\propto \tau^2$. Therefore we can approximate
    ...
    can find the comoving distance $x_{rec}=280\Mpc$.
