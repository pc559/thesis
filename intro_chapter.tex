...
\subsection{Probability to observations}\label{corr_functions}
    \textcolor{red}{
    ...
    which we will test these theories will be predictions of ensemble averages, expectation
    values of measurements applied to some particular draw from the predicted probability distribution.
    The ensemble average~\eqref{expectation_value_field} of a product of the value of
    the same field at two different point is known as the
    two-point correlator, or the two-point function $\left<f(\mathbf{x})f(\mathbf{x'})\right>$.
    Here, the brackets $\left<\ldots\right>$ refer to the expected value over the ensemble,
    over all the possible realisations that could be drawn from the probability distribution
    predicted by the inflation model, weighted by their probability.


    There are two assumptions we wish to immediately impose on this function, and higher order correlators
    (expectation values of products of more fields).
    ...
    This does not mean that some realisation of the ensemble is forbidden from having special points---fields
    ...
    All observations of the universe so far appear to respect this symmetry of the statistics.
    ...
    has $9$ apparent degrees of freedom, but $B(k_1,k_2,k_3)$ has only three. This is due to the delta function
    ...
    inhomogeneities in the very early universe.
    This defines the initial conditions for the $\lcdm$ evolution that follows---from
    this, we can calculate the statistical properties of the $\cmb$ sky that we see.


    Another symmetry we could imagine is statistical scale invariance.
    Scale invariance is the statement that
    \begin{align}
        \left<f(\mathbf{x})f(\mathbf{x'})\right> = \left<f(\lambda\mathbf{x})f(\lambda\mathbf{x'})\right>.
    \end{align}
    From \textcolor{red}{calculation}, we can see that scale invariance needs
    $P(k)\propto k^{-3}$.
    ...
    distribution are \textcolor{red}{very small. Is this the right way to think about it?}
    Given that we only have access to one universe, it is reasonable to ask how we could
    possibly make such a strong statement. We will discuss the resolution of this in the
    next section.
...
\subsection{Observations to probability}
\textcolor{red}{Checking if dice are fair.
Organise this!!! Observations to probability.}
...
\textcolor{red}{Organise this!!!}
    ...
    \textcolor{red}{We now present a summary\ldots}
    ...
    calculate the mean and standard deviation of $100$
    coin flips, we would use~\eqref{expectation_value_discrete}.
...
is in determining the feasibility of the overall method.
The initial result here was identifying the contributions of the non-physical configurations
as a novel problem to this method, and identifying basis choice as the key method of overcoming this difficulty.
...
A side-effect of this desire for a separable primordial bispectrum,
...
These methods are implemented in the $\primodal$ code.
    ...
    This constraint is then translated into a constraint on
    a fundamental parameter in the context of an inflationary parameter scan.


    The thesis is organised as follows. 


\textbf{Chapter~\ref{chapter:intro_bispectra}} is an introduction to the bispectrum as an observable.
Brief reviews
of the various parts of the pipeline that connects inflation scenarios to observations
through the bispectrum are presented.
The usual paradigm of bispectrum estimation in the CMB is reviewed,
...
is outlined, and the point at which the separable formalism diverges is
highlighted.
...
which is distinct and separate from our numerical methods, but nonetheless vital.
...
on the sound speed of the $\dbi$ model, which validate our pipeline against a constraint
from the $\planck$ analysis.
This chapter will describe the parametrisation of the scan, and the physical
interpretation of the results.


\textbf{Chapter~\ref{chapter:conclusion}} discusses possible avenues of future work, and presents our conclusions.
