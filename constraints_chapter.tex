...
\caption{
    We plot the relative error in the value of $\fnl$ obtained for the $\planck$ map
    across the scan.
    We also plot the relative error in the standard deviation of the $\fnl$ values obtained for
    the Gaussian simulations used in~\cite{Sohn_2021}. Both relative errors are comparing the
    value obtained for $\Pmax=25$ with that obtained for $\Pmax=30$.
    We see that the convergence across the majority of the scan is better than that of the
    convergence in the primordial bispectrum, as plotted in figure~\ref{fig:dbi_primodal_scan_template_corrs_log30},
    validating the convergence of our pipeline as a whole in $\Pmax$.
}\label{fig:dbi_primodal_scan_template_corrs_p1ns}
\end{figure}
