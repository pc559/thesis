...
\section{The $\primodal$-$\cmbbest$ pipeline}
    ...
    \textcolor{red}{Link to other discussion.}
...
\section{$\dbi$ sound speed constraint}
    ...
    \textcolor{red}{How is this a validation? What level of disagreement would have been too much?}
    Our pipeline results in a $1\sigma$ constraint of $\fnl^{DBI}=27\pm58$,
    whereas the $\modal$ result was $\fnl^{DBI}=46\pm58$. This is a difference of $0.33\sigma$
    \textcolor{red}{check all this}.
    In Appendix B of~\cite{Planck_NG_2013}, the expected scatter of an optimal(?) estimator with an
    imperfect expansion was calculated. Our expansion has a correlation of \textcolor{red}{what}
    with the DBI template, whereas the $\modal$ result used an expansion with a correlation of $95\%$.
    In~\cite{Planck_NG_2013} it was found that this would be expected to lead to a scatter of approximately
    $0.3\sigma$. Thus, if our result was found to differ from the previous constraint by much larger than this
    it would not be a confident validation of our pipeline. Since we find it differs by almost exactly this value,
    we conclude that our result functions as a validation of the pipeline.
    \textcolor{red}{Tighten all this, make quantitative.}
