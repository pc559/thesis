%\input{commands}
\chapter{An Inflationary Constraint from the Bispectrum}\label{chapter:constraints}
\section{Connecting to $\cmbbest$}
    In this chapter, we validate the full $\primodal$-$\cmbbest$ pipeline
    using the $\planck$ data to place a constraint on the sound speed in DBI inflation,
    and comparing it to the same result obtained from $\planck$.
    ...
    and the sound speed (specifically $c_s^*$, the sound speed at horizon crossing of
    the pivot scale) is a calculated quantity.
    We obtain the full shape and amplitude information for
    each scenario in our scan (within our $k$-range) and the final
    constraint makes use of the entirety of that information, up to
    convergence in $\Pmax$.


    In~\cite{Planck_NG_2018}, equation (55), the $\cmb$ bispectrum was used to
    place a constraint on the sound speed during DBI inflation:
    \begin{align}
    ...
    We can however understand the expected scatter by simply examining the
    scatter between the estimators used in the $\planck$ analysis, as
    we show in figure~\ref{fig:equil_constraints_comparison}.
