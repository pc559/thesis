...
\section{Conclusions}
    ...
    as it is tied to a particular scaling it is not an ideal solution. This is due the desire to create
    a full pipeline, which connects to real $\cmb$ data using $\cmbbest$, and the constraint
    that the expense of this code (for reasonable large $\Pmax$) limits the full pipeline to one (or very few) basis sets.
    ...
    such as the parafac method described in~\cite{tensor_decomp_review}. Given the output of $\primodal$,
    the coefficients $\alpha_{pqr}$, one could imagine performing a tensor decomposition on $\alpha_{pqr}$,
    expressing it as a sum of outer products of vectors.
    ...
    models, as this would effectively have a much smaller $\Pmax$.
