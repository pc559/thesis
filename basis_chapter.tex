...
oscillations at early times.
    ...
    \textcolor{red}{Make $\zeta$ vs.\ $u_k$ vs.\ $v_k$ consistent throughout! Hats, maybe?}
...
of basis, but the methods we will describe will be practical for
any basis.
...
$w^{(i)}(\tau)=(-2i)(-2\lambda(\tau)a^4(\tau))$.
The remaining terms then give
\begin{align}
    F^{(i)}(\tau,k) = G^{(i)}(\tau,k) = J^{(i)}(\tau,k) = k^2u_k(0)\dot{u}_k^*(\tau)
\end{align}
where the $k^2$ comes from the definition of the shape function~\eqref{shapefn},
and we obtain $u_k(\tau)$ by numerically solving the equations of motion.
...
\tilde{\alpha}_P^{(i)} =  \tilde{\alpha}_{prs}^{(i)} 	= \Re\int d\tau\, w^{(i)}(\tau)\, f_p^{(i)}(\tau) \,g_r^{(i)}(\tau) \,h_s^{(i)}(\tau)\,,
\end{align}
...
\subsection{Large non-physical contributions}\label{sec:large_non_physical}
...
i.e.\ defining $\Linvk$ such that the first $\Pmax-1$ basis functions match $\Lbasic^{\Pmax-1}$,
...
In order to compare the efficacy of these four \textcolor{red}{update all this!} different basis
function sets ($\Lbasic$, $\Fbasic$, $\Linvk$ and $\Finvk$),
...
\caption{
    \textcolor{red}{Emphasise these are for $1:1000$.}
...
\section{The $\scalingbasis$ basis}\label{sec:scaling_definition}
    ...
    with
    \textcolor{red}{Need better notation!}
...
\section{The $\resobasis$ basis}\label{sec:resobasis_definition}
    ...
    \begin{figure}[!pth]
        ...
        \caption{
            ...
            which functions they are augmented by.
