...
oscillations at early times.
    ...
    \textcolor{red}{Don't talk about large non-physical contributions here, just the cube.}
...
\begin{eqnarray}\label{inin_kdep}
    &&I^{(i)}(k_1,k_2,k_3)\nonumber\\
    &=& \Re\left[v^{(i)}(k_1, k_2,k_3)\int d\tau\, w^{(i)}(\tau)\, \sum_p f_p^{(i)}(\tau) q_p(k_1)\sum_r g_r^{(i)}(\tau) q_r(k_2)\sum_s h_s^{(i)}(\tau) q_s(k_3) + \text{cyclic perms}\right]\nonumber\\	
	&=& v^{(i)}(k_1, k_2,k_3)\sum_{prs}\left(\Re\int d\tau\, w^{(i)}(\tau)\, f_p^{(i)}(\tau)\, g_r^{(i)}(\tau) \,h_s^{(i)}(\tau)\right)\threeqs + \text{cyclic perms}\,.
\end{eqnarray}
...
\section{Large non-physical contributions}\label{sec:large_non_physical}
