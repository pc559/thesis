...
\subfloat[Reconstructing the Maldacena Template]{\label{fig:a}\includegraphics[width=.75\columnwidth]{pmax_reduction_plots_1_1000/malda.png}}\\
\subfloat[Reconstructing the DBI Template]{\label{fig:b}\includegraphics[width=.75\columnwidth]{pmax_reduction_plots_1_1000/dbi.png}}
\caption{
...
\subfloat[$\cos(f(k_1+k_2+k_3))$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{pmax_reduction_plots_1_1000/cos15.png}}\\
\subfloat[$\cos(f(k_1+k_2+k_3))S^{DBI}$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{pmax_reduction_plots_1_1000/dbi_cos15.png}}
\caption{
...
\subfloat[$\cos(f\log(k_1+k_2+k_3))S^{DBI}$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{pmax_reduction_plots_1_1000/dbi_cos5_log.png}}
\caption{
...
\subfloat[$\kmax/\kmin=550$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{pmax_reduction_plots_1_550/dbi_scaling.png}}\\
\subfloat[$\kmax/\kmin=1000$]{\label{fig:b}\includegraphics[width=.75\columnwidth]{pmax_reduction_plots_1_1000/dbi_scaling.png}}\\
\caption{
...
\begin{figure}[!pth]
\centering
\subfloat[$\cos(\omega(k_1+k_2+k_3))$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{freq_scan_plots/cos_freq_scan.png}}\\
\subfloat[$\cos(\omega\ln(k_1+k_2+k_3))$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{freq_scan_plots/cos_log_freq_scan.png}}\\
\caption{
...
\begin{figure}[!pth]
\centering
\subfloat[$\cos(\omega(k_1+k_2+k_3))S^{DBI}$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{freq_scan_plots/dbi_cos_freq_scan.png}}\\
\subfloat[$\cos(\omega\ln(k_1+k_2+k_3))S^{DBI}$]{\label{fig:a}\includegraphics[width=.75\columnwidth]{freq_scan_plots/dbi_cos_log_freq_scan.png}}\\
\caption{
...
\section{The $P_{max}$-$k_{ratio}$ tradeoff}
    ...
    DBI template~\eqref{dbi_ns_shape}, for $\kmax/\kmin=550$ and
...
\section{Scaling basis}
    While the power spectrum information can give us an estimate of the scaling of
    the bispectrum, different models can have different bispectrum scalings,
    despite having the same power spectrum scaling.
    To take this into account, we would like a basis that could fit
    a range of fractional powers of $k$.
    We achieve this using the $\scalingbasis$ basis.
    The $\scalingbasis$ basis is the Legendre polynomials,
    augmented (in the sense of~\eqref{gram_schmidt}) with $k^{-1}$, $\ln(k)k^{-1}$.
    This is motivated by
    \begin{align}
        k^\varepsilon &= e^{\varepsilon\ln k}\\
                      &\approx 1+\varepsilon\ln k
    \end{align}
    when $\varepsilon\ln\left(\kmax\right)$ is sufficiently small.
    \newpage
\section{Resonant basis}
    We also describe a basis specifically designed to capture logarithmic oscillations.
    This goes against our philosophy of desiring a basis that is not tied to
    any specific shape, as $\cmbbest$ is expensive but need only be run once.
    However, logarithmic oscillations are an important type of feature in the literature~\cite{Planck_NG_2018},
    and running $\cmbbest$ twice, once for general features and once for logarithmic oscillations
    may be a viable strategy to cover a very broad range of models.


    The $\resobasis$ basis is based on the Legendre polynomials:
    \begin{align}
        \frac{\mathcal{P}_n(\bar{k})}{\sqrt{k}}\text{, with }\bar{k}=\frac{2\ln(k)-\ln(\kmin\kmax)}{\ln(\kmax)-\ln(\kmin)}
    \end{align}
    where the factor of $\frac{1}{\sqrt{k}}$ is needed to retain orthogonality.
    \newpage
