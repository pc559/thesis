...
The highly oscillatory early-time contributions that are usually killed off by the $i\varepsilon$ prescription
are here avoided as they only contribute to neglected higher-order terms in the expansion
(for a reasonable choice of basis).
For any bispectrum result that can be captured to sufficient precision for some given $\Pmax$,
one can always push the starting time for the numerical integration earlier, to ensure these non-physical
contributions do not affect our result. Two examples of this will be presented in Figure~\ref{fig:plot_time_integrand_scan}.
Understanding can be gained by considering the explicit example on the left hand side of~\eqref{ieps_integral}.
Performing the integral, and then considering the basis expansion of the result, we see that the contributions
from the early-time boundary only affect coefficients of higher order than any we would need to consider.
\textcolor{red}{Check.}
