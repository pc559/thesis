...
\section{Integration weights}
    ...
    At each timestep we must perform a decomposition~\eqref{mode1Dcoeffs_integral},
    given the values of the $k$-modes at these given sample points.
    ...
    \textcolor{red}{Merge}
A previously mentioned, there are two kinds of integrals we must compute
in our set-up: integrals over time of
...
integrals accurately and efficiently.
    ...
    the cost of performing an extra dot product.
...
the integrand will be highly oscillatory (in $N$) with a slowly varying amplitude.
...
The highly oscillatory nature of the mode functions (in $k$) in these early times ($\lvert k\tau_s\rvert\gg1$) suppresses the
...
(i.e.\ the naive method of starting the integrand with a sharp cut)
...
however, validating the accuracy and usefulness of this method.
