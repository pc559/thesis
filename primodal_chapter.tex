\chapter{Separable Methods and Validation}\label{chapter:methods}
...
\section{Numerics of mode evolution}
    ...
    The Bunch-Davies initial conditions for each mode are set sufficiently early that the modes
...
\section{Integration weights}
    ...
    The integrals over time also require many integrals over a fixed set of sample points---one
    integral for each of the shape coefficients, per vertex, over the decomposition
...
One could expect that this forces us to calculate its basis expansion
...
we need only compute the first $\Pmax$ of the coefficients\footnote{
    ...
    and~\eqref{V_definition}, which mixes the modes---see discussion in Section~\ref{sec:setting_notation}.
}
...
Since calculating each extra point in the time integrand requires a
...
Thus the damping $e^{-\beta^2{(\tau_s-T)}^2}$ for $\tau_s<T$ smoothly
...
The point $\tau_s=\tau_s^{cut}$ is the earliest point we calculate the integrand for,
...
%This means that we expect the earliest resonance time to be
...
It is important to note that this tapering method is not the application of the $i\varepsilon$ prescription. The $i\varepsilon$ prescription comes into these methods through the separation of high-frequency and low-frequency effects by the basis choice. The early time contributions which are usually damped by the $i\varepsilon$ prescription are, in our methods, pushed to the coefficients of higher-order basis elements as the early-time integration limit is taken to the infinite limit. This automatic inclusion of the prescription was noticed in~\cite{Funakoshi}, but the reason was not made explicit, and is relevant for considerations of basis choice.
...
Finally, we mention some points relating to including the spatial derivatives in
the calculation.
...
\begin{figure}[h!]
\centering
    \subfloat{\includegraphics[width=\columnwidth]{plots/slice_explained.png}}
\caption{
    ...
    This can be compared to Figure~\ref{fig:tetrapyd_diagram}.
...
in Chapter~\ref{chapter:decomp}. Our accurate match to these shapes validates our
...
second scenario in Figure~\ref{slice_plot_malda}.
...
\section{Conclusions}
    ...
    This allows us to ({\romannumeral 1}) preserve the intrinsic separability of the tree-level
